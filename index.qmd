...
```{r}
#| label: set up - V1_Dashboard_setup.R
#| include: false
#| warning: false
...
# --- Load map background ---
map_path <- "Assets/Stable/WorldMap_scale.png"
...
# --- Define approximate x/y positions for each region (0–1 scale) ---
# Mapping from region names to their Quarto file names
region_file_map <- c(
  "South America" = "pages/southamerica.qmd",
  "Caribbean" = "pages/caribbean.qmd",
  "Pacific Islands" = "pages/pacificislands.qmd",
  "South Asia" = "pages/southasia.qmd",
  "North & Central America" = "pages/centralamericamexico.qmd",
  "Sub-Saharan Africa" = "pages/sub-saharanafrica.qmd",
  "East & Southeast Asia" = "pages/eastsoutheastasia.qmd",
  "Europe, Middle East & North Africa" = "pages/europemiddleeastnorthafrica.qmd"
)
...
region_positions <- data.frame(
  ...
  # ---- Mobile X positions (adjusted to prevent overlap) ----
  mobile_x = c(
    0.20,  # South America - same
    0.28,  # Caribbean - slightly right to avoid North & Central America
    0.88,  # Pacific Islands - slightly left
    0.72,  # South Asia - slightly right
    0.15,  # North & Central America - further right to avoid Caribbean
    0.45,  # Sub-Saharan Africa - same
    0.88,  # East & Southeast Asia - much further right to avoid Europe
    0.50   # Europe, Middle East & North Africa - further left to avoid East Asia
  ),
  
  # ---- Mobile Y positions (adjusted to prevent overlap) ----
  mobile_y = c(
    0.25,  # South America - same
    0.55,  # Caribbean - below North & Central (0.65) and above Sub-Saharan (0.45)
    0.35,  # Pacific Islands - same
    0.50,  # South Asia - moved up to about where East & Southeast was
    0.65,  # North & Central America - further down to avoid Caribbean
    0.45,  # Sub-Saharan Africa - same
    0.64,  # East & Southeast Asia - up, only slightly lower than Europe (0.68)
    0.68   # Europe, Middle East & North Africa - further down
  )
)
...
<img src="Assets/Stable/logo_large.png" alt="Global Dengue Observatory" class="hero-logo">
...
ggsave("Assets/Dynamic/world_plot.png", 
world_plot + theme_void() + theme(legend.position = "none"),
...
if (nrow(world_summary) > 0) {
  ...
  global_latest_ratio <- if (!is.na(latest_ratio_value)) {
    glue::glue("{sprintf('%.1f', latest_ratio_value)}× the seasonal baseline")
  } else {
    "close to the seasonal baseline"
  }
  
  world_ytd <- world_summary %>%
    dplyr::filter(Year == current_year, Month <= recent_month)
  global_ytd_cases <- sum(world_ytd$cases, na.rm = TRUE)
  global_ytd_cases_text <- format(round(global_ytd_cases), big.mark = ",", scientific = FALSE)
  ytd_expected <- sum(world_ytd$Ave_season_monthly_cases, na.rm = TRUE)
  global_ytd_ratio <- if (!is.na(ytd_expected) && ytd_expected > 0) {
    glue::glue("{sprintf('%.1f', global_ytd_cases / ytd_expected)}× the expected cumulative burden")
  } else {
...
<!-- =============================================== -->
<!-- SECTION B — NARRATIVE WITH STICKY GLOBAL PLOT  -->
<!-- =============================================== -->
...
<div class="section-scroll-banner">
  <p class="section-scroll-text">Scroll to explore</p>
</div>
...
knitr::asis_output('<div class="scrolly-step" data-step="2">
  ...
  <img src="Assets/Stable/how_to.png" alt="How to read plots guide" class="howto-image">
</div>')
...
```
  ...
  <!-- RIGHT SIDE — sticky global plot -->
<div class="scrolly-visual">
<div id="global-plot-wrapper">
<img src="Assets/Dynamic/world_plot.png" alt="World dengue plot"/>
</div>
</div>
...
p_map <- ggplot() +
  annotation_custom(
    map_grob,
    xmin = 0, xmax = img_ratio,   # 👈 match the image’s natural width
    ymin = 0, ymax = 1
  ) +
  coord_cartesian(
    xlim = c(0, img_ratio),
...
# ----------------------------
# Clear stale PNGs to avoid macOS case-insensitivity clashes on case-sensitive servers
if (dir.exists("Assets/Dynamic/region")) {
  old_files <- list.files("Assets/Dynamic/region", pattern = "\\.png$", full.names = TRUE)
  if (length(old_files) > 0) invisible(file.remove(old_files))
}
...
for(i in seq_len(nrow(region_positions))){
  ...
  if(region_name %in% names(region_plot_list)){
    plot_file <- paste0("Assets/Dynamic/region/", tolower(gsub(" & |, | ", "-", region_name)), ".png")
    
    # Ensure the directory exists
    if(!dir.exists("Assets/Dynamic/region")) dir.create("Assets/Dynamic/region", recursive = TRUE)
...
# ----------------------------
# Compute ring hex colours for mobile dots (matching Radial.R gradient)
# ----------------------------

library(scales)

# Create gradient palette matching Radial.R scale_fill_gradient2
# low = "green", mid = "yellow", high = "red", midpoint = 1, limits = c(0.5, 2)
ring_colour_pal <- scales::gradient_n_pal(
  colours = c("green", "yellow", "red"),
  values = scales::rescale(c(0.5, 1.0, 2.0))
)

ring_colour_fn <- function(ratio) {
  clamped <- pmin(pmax(ratio, 0.5), 2.0)
  pal_value <- scales::rescale(clamped, from = c(0.5, 2.0))
  ring_colour_pal(pal_value)
}

# Join region_positions with region_callouts to get ytd_ratio
if (exists("region_callouts") && nrow(region_callouts) > 0) {
  region_positions <- region_positions %>%
    left_join(
      region_callouts %>% select(Region, ytd_ratio),
      by = "Region"
    ) %>%
    mutate(
      cum_ratio_capped = pmin(pmax(ytd_ratio, 0.5), 2),
      ring_hex = ifelse(
        !is.na(cum_ratio_capped),
        ring_colour_fn(cum_ratio_capped),
        "#f9a825"  # fallback amber if no data
      )
    )
} else {
  # Fallback if region_callouts not available
  region_positions$ring_hex <- "#f9a825"
}
...
for(i in seq_len(nrow(region_positions))){
  ...
  mobile_x_pct <- region_positions$mobile_x[i] * 100
  mobile_y_pct <- (1 - region_positions$mobile_y[i]) * 100
  ring_hex <- region_positions$ring_hex[i]
  
  plot_html <- sprintf(
    '<a href="%s" class="region-plot" style="left:%f%%; top:%f%%; --ring-color:%s; --mobile-left:%f%%; --mobile-top:%f%%;">
      <img src="%s" alt="%s summary plot">
      <span class="region-plot-label">%s</span>
    </a>',
    region_page, x_pct, y_pct, ring_hex, mobile_x_pct, mobile_y_pct,
    region_positions$plot_file[i], region_name, region_name
  )
  cat(plot_html)
}
...
<div style="text-align: center; margin: 1.5rem 0;">
  <a href="pages/country-index.html" class="btn btn-outline-primary" style="font-size: 1rem; padding: 0.75rem 1.5rem;">

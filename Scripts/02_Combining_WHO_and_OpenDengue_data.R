...
Comparing_coverage_between_WHO_and_OpenDengue <- source("Scripts/01_Choosing_WHO_or_OpenDengue_data_by_country_year.R", 
...
#--------------- Cleaning data 
source("Scripts/00_FUN_WHO_data_cleaning.R", local = environment())
WHO_data_clean <- WHO_data_cleaning(WHO_data)

#--------------- Filtering OpenDengue and WHO datasets for chosen years 

# OpenDengue 
source("Scripts/00_FUN_extracting_desired_OpenDengue_data.R", local = environment())
...
# WHO
source("Scripts/00_FUN_extracting_desired_WHO_data.R", local = environment())
...
source("Scripts/00_FUN_deduplicating_OpenDengue_data.R", local = environment())
...
# Interpolate
source("Scripts/00_FUN_interpolating_missing_OpenDengue_data.R", local = environment())
...
source("Scripts/00_FUN_disaggregating_OpenDengue_weeks_crossing_months.R", local = environment())
...
source("Scripts/00_FUN_weekly_to_monthly_aggregation_OpenDengue.R", local = environment())
...
source("Scripts/00_FUN_prep_OpenDengue_data_to_combine_with_WHO.R", local = environment())
...
source("Scripts/00_FUN_interpolating_missing_WHO_data.R", local = environment())
...
source("Scripts/00_FUN_prep_WHO_data_to_combine_with_OpenDengue.R", local = environment())
...
source("Scripts/00_FUN_clean_WHO_OpenDengue_combined_data.R", local = environment())
...
source("Scripts/00_FUN_filtering_OpenDengue_WHO_combined_data.R", local = environment())

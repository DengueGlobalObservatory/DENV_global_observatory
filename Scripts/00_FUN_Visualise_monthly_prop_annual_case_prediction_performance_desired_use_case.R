#' ---
#' title: "00_FUN_Visualise_monthly_prop_annual_case_prediction_performance_desired_use_case"
#' author: "K Joshi"
#' 
#' ---
#' 
...
Visualise_monthly_prop_annual_case_prediction_performance_desired_use_case <- function(x){
  ...
  #--------------- Correlation of actual total seasonal cases vs predicted seasonal cases
  ...
  x_cor_data <- x %>% 
    ...
    summarise(Acc_vs_pred_total_cor = cor(Total_season_cases, Predicted_seasonal_cases)) %>% 
    arrange(Acc_vs_pred_total_cor) 
  x_cor_data$Country = factor(x_cor_data$Country, levels = unique(x_cor_data$Country))

 acc_vs_pred_annual_cases_cor_scatterplot <- ggplot(x_cor_data) + 
   ...
   ggtitle("Comparing actual and predicted seasonal total cases")
 
 #----- Heatmap
 
 pred_vs_acc_season_total_cases_cor_plot <- ggplot(x_cor_data) + 
   ...
   ggtitle("Correlation between actual and predicted seasonal total cases") +
   theme_minimal() +
   plot_theme
 ...
 acc_vs_pred_annual_cases_rmse_scatterplot <- ggplot(x_rmse_data) + 
 ...
 results <- list(cor_scatterplot = acc_vs_pred_annual_cases_cor_scatterplot,
                 cor_heatmap = pred_vs_acc_season_total_cases_cor_plot,
                 rmse_scatterplot = acc_vs_pred_annual_cases_rmse_scatterplot,

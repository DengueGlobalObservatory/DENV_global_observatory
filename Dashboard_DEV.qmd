...
all_country_plots <- lapply(split_by_country, function(df_country) {
  make_radial_plot(df_country)   # your plotting function that takes a country's df
})
...
#| label: North & Central America snap shots
#| echo: false
region_name <- "North & Central America"
country_order <- data %>%
  filter(Region == region_name) %>%
  distinct(Country) %>%
  pull(Country)
...
observatory’s reach and allow outbreak forecasts to reach the country

...
if (!is.na(country_name_col)) {
  country_page_registry <- read.csv("pages/country/country-config.csv", check.names = FALSE, stringsAsFactors = FALSE) %>%
    dplyr::mutate(
      country_label_key = tolower(trimws(country_name)),
      country_href = paste0("country/", trimws(slug), ".html")
    ) %>%
    dplyr::select(country_label_key, country_href)
  ...
  all_countries_data <- all_countries_data %>%
    dplyr::mutate(country_label_key = tolower(trimws(country_label))) %>%
    dplyr::left_join(country_page_registry, by = "country_label_key")
} else {
...
Explore all countries with available dengue surveillance data. Use the search and sort controls below to filter by name, region, cumulative cases, or severity ratio. Click any radial plot to open that country's detailed summary page.
...
if (nrow(all_countries_data) == 0) {
  ...
  country_cards <- purrr::map(seq_len(nrow(all_countries_data)), function(idx) {
    ...
    country_href <- row$country_href[[1]]
    ...
    } else if (!is.na(country_href) && nzchar(country_href)) {
      htmltools::tags$a(
        href = country_href,
    ...
    # No-data countries get -1 so they sort below 0 when using "Cumulative Cases (Low to High)"
    has_cases <- row$has_current_year_cases[[1]]
    cum_cases_for_sort <- if (!isTRUE(has_cases)) -1 else ifelse(is.na(cum_cases), 0, cum_cases)
    htmltools::div(
      class = "snapshot-card severity-card country-card",
      `data-country-name` = tolower(country_name),
      `data-region` = tolower(region_name),
      `data-cum-cases` = cum_cases_for_sort,
